...
export default function LoanApplicationDocument({ open, onClose, loan }: Props) {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html><head><title>Loan Application - ${loan?.application_number}</title>
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        table.info td.label { background: #f9f9f9; font-weight: 600; color: #555; width: 30%; }
        ...
        .sig-sub { font-size: 11px; color: #888; }
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div ref={printRef} style={{ fontFamily: "'Segoe UI', sans-serif", fontSize: "13px", color: "#1a1a1a", lineHeight: "1.5" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "#555", marginBottom: "16px", padding: "6px 10px", background: "#f5f5f5", borderRadius: "4px" }}>
            ...
            <span><strong>Date:</strong> {loan.application_date}</span>
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12.5px" }}>
              <tbody>
                ...
                <tr>
                  <td style={{ padding: "5px 8px", border: "1px solid #ddd", background: "#f9f9f9", fontWeight: 600, color: "#555" }}>Purpose</td>
                  <td colSpan={3} style={{ padding: "5px 8px", border: "1px solid #ddd" }}>{loan.purpose || "—"}</td>
                </tr>
                <tr>
                  <td style={{ padding: "5px 8px", border: "1px solid #ddd", background: "#f9f9f9", fontWeight: 600, color: "#555" }}>Proposed Start Date</td>
                  <td style={{ padding: "5px 8px", border: "1px solid #ddd" }}>{loan.proposed_start_date || "—"}</td>
                  <td style={{ padding: "5px 8px", border: "1px solid #ddd", background: "#f9f9f9", fontWeight: 600, color: "#555" }}>Remarks</td>
                  <td style={{ padding: "5px 8px", border: "1px solid #ddd" }}>{loan.remarks || "—"}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            {guarantors.length > 0 ? (
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12.5px" }}>
                <thead>
                  <tr>
                    <th style={{ padding: "5px 8px", border: "1px solid #ddd", background: "#f9f9f9", fontWeight: 600, color: "#555", textAlign: "left" }}>#</th>
                    <th style={{ padding: "5px 8px", border: "1px solid #ddd", background: "#f9f9f9", fontWeight: 600, color: "#555", textAlign: "left" }}>Full Name</th>
                    <th style={{ padding: "5px 8px", border: "1px solid #ddd", background: "#f9f9f9", fontWeight: 600, color: "#555", textAlign: "left" }}>Employee ID</th>
                    <th style={{ padding: "5px 8px", border: "1px solid #ddd", background: "#f9f9f9", fontWeight: 600, color: "#555", textAlign: "left" }}>Department</th>
                    <th style={{ padding: "5px 8px", border: "1px solid #ddd", background: "#f9f9f9", fontWeight: 600, color: "#555", textAlign: "left" }}>Signature</th>
                  </tr>
                </thead>
                <tbody>
                  {guarantors.map((g, i) => (
                    <tr key={g.id}>
                      ...
                      <td style={{ padding: "5px 8px", border: "1px solid #ddd", minWidth: "120px" }}></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <p style={{ fontSize: "12.5px", lineHeight: 1.7 }}>
              I, <strong>{loan.employees?.full_name}</strong>, hereby declare that the information provided in this application is true and correct.
              I agree to abide by the terms and conditions of the loan, including repayment obligations as per the schedule.
              I authorize <strong>{companyName}</strong> to deduct the monthly installment from my salary for the duration of the loan period.
            </p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "24px", marginTop: "40px" }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ borderTop: "1px solid #333", marginTop: "50px", paddingTop: "4px", fontSize: "12px" }}>Applicant</div>
              <p style={{ fontSize: "11px", color: "#888", marginTop: "2px" }}>{loan.employees?.full_name}</p>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ borderTop: "1px solid #333", marginTop: "50px", paddingTop: "4px", fontSize: "12px" }}>Department Head</div>
              <p style={{ fontSize: "11px", color: "#888", marginTop: "2px" }}>Name & Signature</p>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ borderTop: "1px solid #333", marginTop: "50px", paddingTop: "4px", fontSize: "12px" }}>Finance Officer</div>
              <p style={{ fontSize: "11px", color: "#888", marginTop: "2px" }}>Name & Signature</p>
            </div>
          </div>
          ...
          <div style={{ marginTop: "20px", paddingTop: "8px", borderTop: "1px solid #ddd", fontSize: "10px", color: "#999", textAlign: "center" }}>
            {companyName} — Loan Application Form | Generated on {today} | This document is system-generated
          </div>
        </div>
      </DialogContent>
    </Dialog>
